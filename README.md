...
2. In your local docs or tutorials repo, remove any existing `torchexpo_sphinx_theme` packages in the `src` folder (there should be a `pip-delete-this-directory.txt` file there)
3. In `requirements.txt` replace the existing git link with a link pointing to your commit or branch, e.g. `-e git+git://github.com/{ your repo }/torchexpo_sphinx_theme.git@{ your commit hash }#egg=torchexpo_sphinx_theme`
...
- [PyTorch (Docs)](https://github.com/torchexpo/pytorch)
- [PyTorch Tutorials](https://github.com/torchexpo/tutorials)
...
In `conf.py` change the html theme to `torchexpo_sphinx_theme` and point the html theme path to this repo's local folder, which will end up looking something like:

```
html_theme = 'torchexpo_sphinx_theme'
html_theme_path = ["../../../torchexpo_sphinx_theme"]
```
